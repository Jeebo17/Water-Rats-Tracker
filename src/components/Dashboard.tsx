...
import { getSessionsForWeek, getUpcomingSessions } from '../util/sessions';
...
const Dashboard: React.FC = () => {
  ...
  const sessionsNeedingLeaders = thisWeekSessions.filter(session => {
    const min = session.minNumberOfLeaders ?? 0;
    return min > 0 && (!session.leaderNames || session.leaderNames.length < min);
  }).length;
          ...
          <div className="space-y-3">
            {weekDays.map(day => {
              const daySession = thisWeekSessions.find(session => 
                format(new Date(session.startTime.toDate()), 'yyyy-MM-dd') === format(day, 'yyyy-MM-dd')
              );
              ...
              return (
                <div key={day.toISOString()} className={`flex items-center justify-between p-2 rounded-lg bg-gray-50 ${isToday ? 'border-blue-600 border-2' : ''}`}>
                  <div className="flex items-center space-x-3">
                    ...
                    {daySession ? (
                      <div>
                        <h3 className="font-medium text-gray-800 text-sm mb-1">{daySession.activity}</h3>
                        <div className="text-xs text-gray-600 space-y-1">
                          ...
                          <div className="flex items-center space-x-2">
                            <span className="flex items-center">
                              ...
                              {daySession.timeTBD ? 'TBD' : format(new Date(daySession.startTime.toDate()), 'h:mm a')}
                            </span>
                            <span className="flex items-center">
                ...
                <div key={session.id} className="p-3 border border-gray-200 rounded-lg">
                  ...
                  <div className="text-xs text-gray-600 space-y-1">
                    <div className="flex items-center">
                      ...
                      {dateObj ? format(dateObj, 'EEEE, MMM d, yyyy') : 'TBD'}
                    </div>
                    <div className="flex items-center">
